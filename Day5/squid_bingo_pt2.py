...
# Now, once a board has five marks it is added to a string of boards to ignore
# So we can keep checking and return the board that wins last
def check_bingo(bingo_numbers, boards, empty_bingo_boards):
    ...
    for n in bingo_numbers:
        ...
        for b in range(num_boards):
            # check if board already got bingo
            # and needs to be ignored
            if ignore_boards.find(str(b)) == -1:
                try:
...
def squid_bingo(bingo_numbers, bingo_boards, zero_boards):
    # gets list of winning numbers and boards
    winning_bn = check_bingo(bingo_numbers, bingo_boards, zero_boards)
    print(winning_bn[len(winning_bn) - 1])
    score = calculate_score(winning_bn[len(winning_bn) - 1], bingo_boards, zero_boards)
    print(score)
